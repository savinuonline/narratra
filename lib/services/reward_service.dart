...
class RewardService {
  ...
  Future<UserReward> getUserRewards() async {
    ...
    if (doc.exists) {
      return UserReward.fromJson(doc.data()!..['userId'] = user.uid);
    } else {
      ...
      await _firestore.collection('rewards').doc(user.uid).set(newReward.toJson());
      return newReward;
    }
  }
  ...
  Future<void> saveUserRewards(UserReward rewards) async {
    await _firestore.collection('rewards').doc(rewards.userId).set(rewards.toJson());
  }
  ...
  Future<void> processReferral(String referrerId) async {
    ...
    final referrerDoc = await _firestore.collection('rewards').doc(referrerId).get();
    
    if (referrerDoc.exists) {
      final referrerRewards = UserReward.fromJson(referrerDoc.data()!..['userId'] = referrerId);
  ...
  Future<void> updateGoalProgress(int progressIncrement) async {
    ...
    final previousProgress = (rewards.dailyGoalProgress - progressIncrement) / rewards.dailyGoal;
